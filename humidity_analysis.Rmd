...
f_p_water <- 1.0016+3.15*(10^-6)*(synoptic_rh_liquid$pressure_mb)-0.074/(synoptic_rh_liquid$pressure_mb)

sat_vap_p_water_pure <- 6.112*(exp(17.62*(synoptic_rh_liquid$dry_bulb)/(243.12+(synoptic_rh_liquid$dry_bulb)))) # saturation vapor pressure for pure water
...
water_vap_p <- 6.112*(exp(17.62*(synoptic_rh_liquid$wet_bulb)/(243.12+(synoptic_rh_liquid$wet_bulb))))*f_p_water - 6.53*(10^-4)*(1+0.000944*(synoptic_rh_liquid$wet_bulb))*(synoptic_rh_liquid$pressure_mb)*(synoptic_rh_liquid$dry_bulb - synoptic_rh_liquid$wet_bulb)
...
nrow(rh[rh$RH<0,])
  # 73 negative out of 121,618 rows
...
spring_rh %>% 
  ...
  annotate("text", x=1955, y=91, label = paste("p-value =", round((spring.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=1955, y=91.5, label = paste("Sen's Slope =", round((spring.sen[["estimates"]][["Sen's slope"]]), digits = 3)))
dev.off()
...
winter_rh %>% 
  ...
  annotate("text", x=2015, y=94.2, label = paste("p-value =", round((winter.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=2015, y=94.5, label = paste("Sen's Slope =", round((winter.sen[["estimates"]][["Sen's slope"]]), digits = 3)))
dev.off()
```
...
clear_summary %>% 
  ...
  annotate("text", x=2015, y=85.8, label = paste("p-value =", round((clear.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=2015, y=86.2, label = paste("Sen's Slope =", round((clear.sen[["estimates"]][["Sen's slope"]]), digits = 3)))
dev.off()
...
# Fog frequency
```{r}
# Fog frequency by season: all days 1935-2022
...
ff_annual_trends_all <- bind_rows(ff_annual_trend, ff_seasonal_trend)  %>% 
  ...
  mutate(" " = ifelse(is.na(season), "Annual", season), 
...
ff_table <- ff_annual_trends_all %>% 
  kbl(caption = "Decadal Trends of Fog Frequency from 1935-2022") %>%
  footnote(general = c("*indicates 95% confidence", "Sen's Slope analysis used to calculate annual fog frequency trends")) %>% 
...
annual_trends_all <- bind_rows(annual, annual_season, annual_clear, annual_season_clear)  %>% 
  mutate(Type = type, 
         ...
         "Decadal trend" = ifelse(p <= 0.05, str_c(decadal_trend, "*"), decadal_trend), # add asterisks
         "p-value" = p) %>% 
  dplyr::select(" ", "Decadal trend", year) %>% 
  distinct() %>%   # removes rows with repeat values
  as.data.frame(html_table())
...
ggplot(all, aes(x = year, y = avg_rh)) + 
  geom_line(data = all, aes(x = year, y = avg_rh), color = "grey") + geom_smooth(data = all, aes(x = year, y = avg_rh), method = "lm", se = F, color = "darkgrey") + 
  geom_line(data = clear, aes(x = year, y = avg_rh), color = "skyblue") + geom_smooth(data = clear, aes(x = year, y = avg_rh), method = "lm", se = F, color = "skyblue3") +
  theme_minimal()
...
rh_table <- annual_trends_all %>%
  kbl(caption = "Decadal Trends of Relative Humidity from 1935-2022") %>%
