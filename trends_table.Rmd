...
trends <- bind_rows(trends_overall, trends_clear, trends_diurnal) %>% 
  mutate(decadal_trend = trend*10,
         decadal_trend = ifelse(abs(decadal_trend) > 0.001, format(round(decadal_trend, 3), nsmall = 3),format(round(decadal_trend,0), nsmall = 0)),
...
trends_table <- trends %>% 
  ...
  footnote(general = c("*indicates 95% confidence", "Sen's Slope analysis used to calculate trends"))
