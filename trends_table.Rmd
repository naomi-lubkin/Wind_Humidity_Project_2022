...
trends <- bind_rows(trends_overall, trends_clear, trends_diurnal) %>% 
  mutate(decadal_trend = trend*10,
         decadal_trend = ifelse(measure != "mr", ifelse(abs(decadal_trend) > 0.001, format(round(decadal_trend, 3), nsmall = 3),format(round(decadal_trend,0), nsmall = 0)),format(round(decadal_trend, 6), nsmall = 6)),
