...
```{r}
#producing an error (sometimes) that says data is not numeric (which it for sure is), so ? 
wind_snht<- snht(wind_spd, period=80, robust=FALSE, time =NULL)
summary(wind_snht)
snht_plot <- plotSNHT(wind_snht, time = NULL, alpha = NULL)
```
