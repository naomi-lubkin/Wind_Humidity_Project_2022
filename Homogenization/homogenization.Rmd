...
hourly_avg_1941 <- hourly_avg_1941 %>% 
  mutate(homogenized_wind = wind_speed * 1.056) # homogenize by multiplying first era by 1.056
...
homogenized %>% 
  group_by(group) %>% 
  summarize(mean = mean(homogenized_wind))
  
```


## Bootstrap the difference in means: ensuring the bootstrapped averages are right
```{r}
...
a <- homogenized %>% 
  group_by(year) %>% 
  summarize(avg_speed = mean(wind_speed), avg_homogen = mean (homogenized_wind)) %>% 
  ggplot(aes(x = year, y = avg_homogen))+ 
  geom_point() + 
  ylim(20,45)
b <- homogenized %>% 
  group_by(year) %>% 
