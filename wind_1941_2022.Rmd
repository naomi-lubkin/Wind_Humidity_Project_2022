...
year_plot_homogenized <- ggplot(data = yearly_avg, aes(x = year, 
                           y = homogenized_wind, color = group)) +
  geom_point() +
  ylim(0,45) +
...
```{r}
#annual
annual_trend <- yearly_avg %>% 
  filter(year >= 1981) %>% 
  summarize(season = "Annual",
            annual_trend = sens.slope(yearly_avg$homogenized_wind, conf.level = 0.95)[["estimates"]],
            p = sens.slope(yearly_avg$homogenized_wind, conf.level = 0.95)[["p.value"]]) 

seasonal_trends <- rbind(spring, summer, fall, winter)  
...
trends_all <- rbind(annual_trend, seasonal_trends) %>% 
...
save_kable(wind_table, file = "Figures/wind_table.pdf")
