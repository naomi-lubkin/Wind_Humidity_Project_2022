...
avgs <- seasonal_avg %>% 
  mutate(season = factor(season, levels = c("Overall", "Winter", "Spring", "Summer", "Fall"))) %>% 
  group_by(season) %>% 
...
avgs %>% 
mutate(season...1 = factor(season...1, levels = c("Overall", "Winter", "Spring", "Summer", "Fall")),
       season...3 = factor(season...3, levels = c("Overall", "Winter", "Spring", "Summer", "Fall")))
...
trends_all_81 <- rbind(annual_trend_81, seasonal_trends_81) %>% 
  mutate(decadal_trend = annual_trend*10,
         ...
         "Decadal trend (mph/decade)" = ifelse(p <= 0.05, str_c(decadal_trend, "*"), decadal_trend), # add asterisks
         "p-value" = p) %>% 
  dplyr::select(" ", "Decadal trend (mph/decade)")
...
seasonal_trends_41 <- seasonal_avg %>%
  group_by(season) %>%
  summarize(day_status = "all",
            annual_trend = sens.slope(wind_speed, conf.level = 0.95)[["estimates"]],
            p = sens.slope(wind_speed, conf.level = 0.95)[["p.value"]])

#diurnal (from diurnal_wind.rmd)
diurnal_trend <- read_csv("Figures/diurnal_wind_trends.csv") %>% 
  dplyr::select(2:5) %>% 
  rename(annual_trend = trend)
  
  

trends_all_41 <- rbind(annual_trend_41, seasonal_trends_41, diurnal_trend) %>%
  mutate(decadal_trend = annual_trend*10,
            decadal_trend = format(round(decadal_trend, 3), nsmall = 3),
            decadal_trend = ifelse(p <= 0.05, str_c(decadal_trend, "*"), decadal_trend)) %>% 
  dplyr::select(!c(annual_trend, p)) %>% 
  pivot_wider(names_from = day_status, values_from = decadal_trend) %>% 
  transmute(" " = season,
         Overall = all,
         Day = day,
         Night = night)
...
wind_table_41 <- trends_all_41 %>%
  ...
  footnote(general = c("*indicates 95% confidence", "Sen's Slope analysis used to calculate annual wind speed trends", "Wind speeds prior to 1980 homogenized to account for the Observatory location shift")) %>%   kable_classic(full_width = F, html_font = "Cambria")
