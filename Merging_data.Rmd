...
pres_weather <- read_csv("Data/b16 hourly present weather split by code 1935-2021 (1).csv")
...
#for 1981-2021 (merging wind with homogenized 1941-2021 wind data is below)
```{r}
#filter for years of interest (1981-2021)
hourly_wind <- hourly_wind_raw %>% 
  filter(year(date) >= 1981)
#filter for other years of interest (1941-2021)
hourly_wind_41 <- hourly_wind_raw %>% 
...
```{r}
#clean data
homogenized_hourly <- hourly_wind_41 %>% 
  filter(year(date) >= 1941) %>%
  mutate(wind_speed = as.numeric(wind_speed)) 
...
merged_data_41 <- merged_data_41 %>% 
  dplyr::select(date, temperature, dry_bulb, wet_bulb, pressure_mb, precipitation, RH,  homogenized_wind)


```

#datasets for each wind threshold, 1941-2021
```{r}
#25mph threshold
...
#making dataframes for each month, overall and for models, 1941-2021 events
```{r}
#25 mph threshold
...
# overall
counts_final25_41_avg <- counts_final25_41 %>% 
  group_by(year) %>% 
  summarize(avg = mean(count_events))
ggplot(counts_final25_41_avg, aes(year, avg)) +
  geom_point() +
  theme_minimal() +
...
t25.sen <- sens.slope(counts_final25_41_avg$avg, conf.level = 0.95)
View(t25.sen)
t25.sen[["p.value"]]
...
# overall
counts_final60_41_avg <- counts_final60_41 %>% 
  group_by(year) %>% 
  summarize(avg = mean(count_events))
ggplot(counts_final60_41_avg, aes(year, avg)) +
  geom_point() +
  theme_minimal() +
  labs(x = 'Year', y = "Number of Events", title = "Number of Events at 60 mph Threshold", subtitle = "1941-2021")

#sens slope for overall
t60.sen <- sens.slope(counts_final60_41_avg$avg, conf.level = 0.95)
View(t60.sen)
t60.sen[["p.value"]]
...
# overall
counts_final100_41_avg <- counts_final100_41 %>% 
  group_by(year) %>% 
  summarize(avg = mean(count_events))
ggplot(counts_final100_41_avg, aes(year, avg)) +
  geom_point() +
  theme_minimal() +
...
#sens slope for overall
t100.sen <- sens.slope(counts_final100_41_avg$avg, conf.level = 0.95)
View(t100.sen)
t100.sen[["p.value"]]
