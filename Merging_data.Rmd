...
```{r}
#25mph threshold
#wind_fog column returns a "Y" for synoptic hours that are  below 32 degrees F, have wind averages for the hour 25mph or above, and have 100% relative humidity (indicates fog). returns N if criteria not met
windfog25_41 <- merged_data_41 %>%
  mutate(wind_fog = ifelse(homogenized_wind >= 25  & RH == 100 & temperature < 32, "Y", "N"))


#60mph threshold
#wind_fog column returns a "Y" for synoptic hours that are  below 32 degrees F, have wind averages for the hour 60mph or above, and have 100% relative humidity (indicates fog). returns N if criteria not met
windfog60_41 <- merged_data_41 %>%
  mutate(wind_fog = ifelse(homogenized_wind >= 60  & RH == 100 & temperature < 32, "Y", "N"))

#100mph threshold
#wind_fog column returns a "Y" for synoptic hours that are  below 32 degrees F, have wind averages for the hour 100mph or above, and have 100% relative humidity (indicates fog). returns N if criteria not met
windfog100_41 <- merged_data_41 %>%
  mutate(wind_fog = ifelse(homogenized_wind >= 100 & RH == 100 & temperature < 32, "Y", "N"))
```
