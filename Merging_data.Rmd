...
#wind data (from wind_1941_2022.rmd, in mph)
hourly_wind <- read_csv("Data/wind_from_wind_1941_2022_rmd.csv") %>% 
  unite("date", 2:3, sep = " ") %>% 
  mutate(date = as.POSIXct(date, t="EST", format = "%Y-%m-%d %H:%M:%OS"))

#temp data (from temperature_analysis.rmd, in celsius)
hourly_temp <- read_csv("Data/temp_from_temperature_analysis_rmd.csv")

#rh data (from humidity_analysis.rmd, in celsius)
...
homogenized_hourly <- hourly_wind %>%
  mutate(homogenized_wind = ifelse(year(date) <=1981, hourly_wind$wind_speed*1.056, hourly_wind$wind_speed)) 
...
```{r}
#25mph threshold
#wind_fog column returns a "Y" for synoptic hours that are  below 0 degrees C, have wind averages for the hour 25mph or above, and have 100% relative humidity (indicates fog). returns N if criteria not met
windfog25_41 <- merged_data_41 %>%
  mutate(wind_fog = ifelse(homogenized_wind >= 25  & RH == 100 & temperature < 0, "Y", "N"))


#60mph threshold
#wind_fog column returns a "Y" for synoptic hours that are  below 0 degrees C, have wind averages for the hour 60mph or above, and have 100% relative humidity (indicates fog). returns N if criteria not met
windfog60_41 <- merged_data_41 %>%
  mutate(wind_fog = ifelse(homogenized_wind >= 60  & RH == 100 & temperature < 0, "Y", "N"))

#100mph threshold
#wind_fog column returns a "Y" for synoptic hours that are  below 0 degrees C, have wind averages for the hour 100mph or above, and have 100% relative humidity (indicates fog). returns N if criteria not met
windfog100_41 <- merged_data_41 %>%
  mutate(wind_fog = ifelse(homogenized_wind >= 100 & RH == 100 & temperature < 0, "Y", "N"))
```
...
```{r}
# checking data
ggplot(merged_data_41 %>% 
  group_by(year = year(date)) %>% 
  summarize(n = n()),
  aes(x = year, y = n))+
  geom_col() 
#1970 overall less counts
  
#events: 25 mph threshold
counts_final25_41 <- windfog25_41 %>% 
  filter(wind_fog == "Y") %>% 
  group_by(month = month(date), year = year(date), wind_fog) %>% 
  summarize(count = n())


#events: 60 mph threshold
counts_final60_41 <- windfog60_41 %>% 
  filter(wind_fog == "Y") %>% 
  group_by(month = month(date), year = year(date), wind_fog) %>% 
  summarize(count = n())
...
counts_final100_41 <- windfog100_41 %>% 
  filter(wind_fog == "Y") %>% 
  group_by(month = month(date), year = year(date), wind_fog) %>% 
  summarize(count = n())
```
...
counts_final25_41_sum <- counts_final25_41 %>% 
  group_by(year) %>% 
  summarize(total_events_yr = sum(count))
...
mph25_41 <- ggplot(counts_final25_41_sum, aes(year, total_events_yr)) +
  geom_smooth(method = "lm", se = F, color = "grey") +
  geom_point() +
  theme_minimal() +
  labs(x = 'Year', y = "Number of Events", title = "Number of Events at 25 mph Threshold", subtitle = "(a) 25 mph Threshold") +
...
mph60_41 <- ggplot(counts_final60_41_sum, aes(year, total_events_yr)) +
  geom_point() +
  theme_minimal() +
  labs(x = 'Year', y = "Number of Events", subtitle = "(b) 60 mph Threshold") +
  annotate("text", x=2015, y=175, label = paste("p-value =", round((t60.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=2015, y=185, label = paste("Sen's Slope =", round(t60.sen[["estimates"]][["Sen's slope"]], digits = 3)))
mph60_41
dev.off()
...
mph100_41 <- ggplot(counts_final100_41_sum, aes(year, total_events_yr)) +
  geom_point() +
  theme_minimal() +
  #scale_y_continuous(breaks = seq(0, 20, by = 1)) +
  labs(x = 'Year', y = "Number of Events", subtitle = "(c) 100 mph Threshold") +
...
all_threshold_1941 <- mph25_41 / mph60_41 / mph100_41

ggsave("Figures/all_threshold_1941.jpeg", plot = all_threshold_1941, width = 8, height = 10, dpi = 1000)
...
wind_fog_25 <- merged_data %>% 
 mutate(wind_fog = ifelse(wind_speed >= 25  & RH >= 96 & temperature < 0, "Y", "N"))



#60 mph wind threshold
wind_fog_60 <- merged_data %>% 
  mutate(wind_fog = ifelse(wind_speed >= 60  & RH >= 96 & temperature < 0, "Y", "N"))




#dataframe with Y in column if wind speeds (hourly average) are 100 or above, temp is below 0 (filtering out =0 to avoid edge cases where the hour may be hovering around/above freezing) and RH is 96% or above (based on RH error in Seidel for fog). returns N if criteria not met
wind_fog_100 <- merged_data %>% 
  mutate(wind_fog = ifelse(wind_speed >= 100  & RH >= 96 & temperature < 0, "Y", "N"))
...
ggplot(counts, aes(year, counts, shape = threshold)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = F, size = 0.5, color = "grey") +
  labs(x = "Year", y = "Number of events", title = "Number of events at each wind speed threshold") + 
  theme_minimal() +
  annotate("text", x=2015, y=40, size = 3, label = paste("p-value =", round((t25_81.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=2015, y=42, size = 3, label = paste("Sen's Slope =", round(t25_81.sen[["estimates"]][["Sen's slope"]], digits = 3))) +
  annotate("text", x=2016, y=15, size = 3, label = paste("p-value =", round((t60_81.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=2016, y=17, size = 3, label = paste("Sen's Slope =", round(t60_81.sen[["estimates"]][["Sen's slope"]], digits = 3))) + 
  annotate("text", x=2017, y=2.5, size = 3, label = paste("p-value =", round((t100_81.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=2017, y=4.5, size = 3, label = paste("Sen's Slope =", round(t100_81.sen[["estimates"]][["Sen's slope"]], digits = 3)))
```
