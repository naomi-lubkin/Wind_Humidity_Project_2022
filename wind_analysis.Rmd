...
#not doing what I want it to here (for some reason is replacing all values with NA, not just Null)
...
stage_office_peak_5_sec_gusts_avgbymin <- stage_office_peak_5_sec_gusts %>% 
  ...
  group_by(date) %>% 
  summarize(avg_setra = mean(setra, na.rm = TRUE), 
...
stage_office_peak_5_sec_gusts%>%
  summarize(avgft742  = mean(ft742, na.rm = TRUE))



#trying same methods on smaller dataset (not working here either)
df <- matrix(c(1,5,6,NA,3,5,NA, 1), ncol=2)
df %>%
  summarize(mean(values, na.rm = TRUE))
```
# Analysis by Month, Using Monthly Averages
```{r}
#November (10)
