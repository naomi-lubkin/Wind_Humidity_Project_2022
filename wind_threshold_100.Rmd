...
winter_above100_summary <- season_above100 %>% 
  group_by(season, year_for_winter) %>% 
  summarize(event_count_total = sum(event_count)) %>% 
  filter(season == "Winter")

# visual
ggplot(winter_above100_summary, aes(x = year_for_winter, y = event_count_total)) +
  geom_point() +
  geom_smooth(method = "lm", se = F, color = "grey") +
  theme_minimal() +
  labs(title = "Winter Events Above 100 mph", subtitle = "1981 - 2021", x = "Year", y = "Number of Events") 

# Sen's slope analysis
winter100.sen <- sens.slope(winter_above100_summary$event_count_total, conf.level = 0.95)
View(winter100.sen)
winter100.sen[["p.value"]]
```
