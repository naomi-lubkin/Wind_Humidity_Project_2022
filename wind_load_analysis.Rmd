...
# ANOTHER WAY TO DO ABOVE COMMAND, but slightly off because of days with "na" windloads (eg there are not always 92 days of data in spring)
...
#                                              TRUE ~ season)) %>%
#     mutate(seasonal_avg_wl = case_when(season == "Winter" ~ sum_wind/90,
#                          season == "winter_leap" ~ sum_wind/91, 
#                          season == "Spring" ~ sum_wind/92,
#                          season == "Summer" ~ sum_wind/92,
#                          season == "Fall" ~ sum_wind/91)) %>%
#   mutate(wl_homogenized =  ifelse(year <=1981, seasonal_avg_wl*1.05, seasonal_avg_wl))
