...
avg <- yearly_avg_t %>% 
  select(year, yearly_avg) %>% 
  mutate(datatype = "Average", avg = yearly_avg)

min_max_avg <- bind_rows(min, max, avg) %>% 
  select(year, avg, datatype) %>% 
  mutate(datatype = factor(datatype, levels = c("Maximum", "Average", "Minimum")))

# Sen's slopes
min.sen <- sens.slope(min_max_avg$avg_min, conf.level = 0.95)
max.sen <- sens.slope(min_max_avg$avg_max, conf.level = 0.95)
avg.sen <- sens.slope(min_max_avg$yearly_avg, conf.level = 0.95)
...
temp_min_max <- ggplot(min_max_avg, aes(year, avg, color = datatype)) +
  ...
  annotate("text", x=1955, y=-4.5, label = paste("p-value =", round((min.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=1955, y=-4, label = paste("Sen's Slope =", round((min.sen[["estimates"]][["Sen's slope"]]), digits = 3))) + # min
  annotate("text", x=1955, y=-1.5, label = paste("p-value =", round((avg.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=1955, y=-1, label = paste("Sen's Slope =", round((avg.sen[["estimates"]][["Sen's slope"]]), digits = 3))) + # avg
  annotate("text", x=1955, y=2.5, label = paste("p-value =", round((max.sen[["p.value"]]), digits = 3))) +
  annotate("text", x=1955, y=3, label = paste("Sen's Slope =", round((max.sen[["estimates"]][["Sen's slope"]]), digits = 3))) + # max
