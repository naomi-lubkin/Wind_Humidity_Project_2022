...
min <- min_max %>% 
  dplyr::select(year, avg_min) %>% 
  mutate(datatype = "Minimum", avg = avg_min)
max <- min_max %>% 
  dplyr::select(year, avg_max) %>% 
  mutate(datatype = "Maximum", avg = avg_max)
avg <- yearly_avg_t %>% 
  dplyr::select(year, yearly_avg) %>% 
  mutate(datatype = "Average", avg = yearly_avg)

min_max_avg <- bind_rows(min, max, avg) %>% 
  dplyr::select(year, avg, datatype) %>% 
  mutate(datatype = factor(datatype, levels = c("Maximum", "Average", "Minimum")))

# Sen's slopes
min_max_avg_trends <- min_max_avg %>% 
  group_by(datatype) %>% 
  summarize(trend = sens.slope(avg, conf.level = 0.95)[["estimates"]][["Sen's slope"]], p_value = sens.slope(avg, conf.level = 0.95)[["p.value"]])


# figures
temp_min_max <- ggplot(min_max_avg, aes(year, avg, color = datatype)) +
  geom_line() +
  theme_minimal() +
  geom_smooth(min_max, mapping = aes(year, avg_min), method = "lm", se = F, color = "grey", size = 0.5) + # min
  geom_smooth(yearly_avg_t, mapping = aes(year, yearly_avg), method = "lm", se = F, color = "grey", size = 0.5) + # avg
  geom_smooth(min_max, mapping = aes(year, avg_max), method = "lm", se = F, color = "grey", size = 0.5) + # max
  scale_color_manual(values = c("darkred", "orange", "lightblue3")) +
  labs(title = "Annual Temperature (°C) trends from 1941-2022", x = "Year", y = "Temperature (°C)") 

ggsave(plot = temp_min_max, filename = "Figures/temp_min_max_annual.jpeg", width = 8, height = 5, dpi = 1000)

```

# Min and Max: seasonal
```{r}
# caluclate seasonal min and max from hourly temperature
min_max_season <- hourly_temp %>% 
  mutate(day = date(date)) %>% 
  group_by(day, season) %>% 
  summarise(minimum = min(temperature), 
            maximum = max(temperature)) %>% 
  group_by(month = month(day), year = year(day), season) %>% 
  summarize(avg_min = mean(minimum), 
            avg_max = mean (maximum)) %>% 
...
min_season <- min_max_season %>% 
  dplyr::select(year, avg_min, season) %>% 
  mutate(datatype = "Minimum", avg = avg_min)
max_season <- min_max_season %>% 
  dplyr::select(year, avg_max, season) %>% 
  mutate(datatype = "Maximum", avg = avg_max)
avg_season <- seasonal_avg_t %>% 
  dplyr::select(year_for_winter, seasonal_avg, season) %>% 
  mutate(datatype = "Average", avg = seasonal_avg, year = year_for_winter)

min_max_avg_season <- bind_rows(min_season, max_season, avg_season) %>% 
  dplyr::select(year, avg, datatype, season) %>% 
  mutate(datatype = factor(datatype, levels = c("Maximum", "Average", "Minimum")))

# Sen's slopes
min_max_avg_season_trends <- min_max_avg_season %>% 
  group_by(season, datatype) %>% 
  summarize(trend = sens.slope(avg, conf.level = 0.95)[["estimates"]][["Sen's slope"]], p_value = sens.slope(avg, conf.level = 0.95)[["p.value"]])

# figures
temp_min_max_season <- ggplot(min_max_avg_season, aes(year, avg, color = datatype)) +
  geom_line() +
  theme_minimal() +
  facet_wrap(~season) +
  # geom_smooth(min_max, mapping = aes(year, avg_min), method = "lm", se = F, color = "grey", size = 0.5) + # min
...
ggsave(plot = temp_min_max_season, filename = "Figures/temp_min_max_seasonal.jpeg", width = 8, height = 5, dpi = 1000)


# Trends table
#seasonal
min_max_avg_season_trends
#annual
min_max_avg_trends <- min_max_avg_trends %>% 
  mutate(season = "Annual")

trends_min_max_all <- bind_rows(min_max_avg_trends, min_max_avg_season_trends)  %>%
  mutate(decadal_trend = trend*10,
         decadal_trend = format(round(decadal_trend, 3), nsmall = 3), # round trend to 3 decimal places
         #"Decadal trend" = ifelse(p_value <= 0.1, str_c(decadal_trend, "\u2020"), decadal_trend), # add dagger
         "Decadal trend" = ifelse(p_value <= 0.05, str_c(decadal_trend, "*"), ifelse(between(p_value, 0.05, 0.1), str_c(decadal_trend, "\u2020"), decadal_trend)), # add asterisk
         "p-value" = p_value,
         " " = datatype) %>%
  dplyr::select(season, "Decadal trend", " ") %>%
  tidyr::spread(season, "Decadal trend") %>% 
  dplyr::select(" ", Annual, Winter, Spring, Summer, Fall)

temp_min_max_table <- trends_min_max_all %>%
  kbl(caption = "Decadal Trends of Temperature (°C) from 1941-2022") %>%
  footnote(general = c("\u2020 indicates 90% confidence", "*indicates 95% confidence", "Sen's Slope analysis used to calculate annual temperature trends")) %>%
  kable_classic(full_width = F, html_font = "Cambria") 

save_kable(temp_min_max_table, file = "Figures/temperature_min_max_table.pdf")
```
