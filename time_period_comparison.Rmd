...
t.test(monthly_avg_spd ~ era, data=all_data_monthly)
#p= 0.001218
...
x <- all_data_monthly$monthly_avg_spd[all_data_monthly$era == 1]
y <- all_data_monthly$monthly_avg_spd[all_data_monthly$era == 2]
wilcox.test(monthly_avg_spd ~ era, data = all_data_monthly)
```
...
```{r}
snht_yearly <- snht(yearly_avg$year_avg_speed, 80, robust=F, time=NULL)
```
