...
hourly_direction <- rbind(hourly_direction, wind_2022)
...
wind_counts <- hourly_dir %>%
  group_by(dir_compass, year) %>%
...
westerly_wind_prop <- wind_counts %>%
  filter(dir_compass == "W")
...
wind.dir.anova <- aov(dir_degree ~ season, data = hourly_dir)
summary(wind.dir.anova)
TukeyHSD(wind.dir.anova)

# ANOVA: wind direction and decade
wind.dir.decade.anova <- aov(dir_degree ~ decade, data = hourly_dir)
summary(wind.dir.decade.anova)

# ANOVA: wind direction and season, decade
wind.dir.seasondecade.anova <- aov(dir_degree ~ decade * season, data = hourly_dir)
summary(wind.dir.seasondecade.anova)
TukeyHSD(winpd.dir.seasondecade.anova)
...
ggplot(data = hourly_dir, aes(x=decade, y = dir_degree)) +
  geom_count() +
  scale_size_area(max_size =10)

ggplot(data = wind_counts, aes(x=year, y = prop)) +
  geom_bar(stat="identity") +
  facet_wrap(~dir_compass)
